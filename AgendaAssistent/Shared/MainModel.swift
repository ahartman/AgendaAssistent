...
@Observable class MainModel {
    ...
    init() {
        ...
        period.periodDates = Period.PeriodStartEnd(
            start: kalender.date(
                byAdding: DateComponents(day: Int(localStart) * 7 + 1),
                to: zeroDate
            )!,
            end: kalender.date(
                byAdding: DateComponents(day: Int(localLength) * 7),
                to: zeroDate
            )!
        )
    ...
    func filterPatientData() -> [PatientInfo] {
        return patientAllVisits.filter {
            $0.visits.contains(where: {
                $0.visitCalendar != "Marieke speciallekes"
            })
        }
    }
    ...
    func startUpdate() {
        ...
        let tempForward =
            kalender.dateComponents([.day], from: zeroDate, to: forwardLimit!)
            .day! / 7
        period.periodEnds = [
            CGFloat(
                -1
    ...
    func loadAndUpdate() {
        patientAllVisits = DBModel().getPatientAllInfo(
            dates: period.periodDates
        )
    ...
    func dailyUpdate() {
        if !kalender.isDateInToday(
            ...
            let dailyDates = Period.PeriodStartEnd(
                start: kalender.date(
                    byAdding: DateComponents(day: -1 * 7),
                    to: zeroDate
                )!,
                end: kalender.date(
                    byAdding: DateComponents(day: 6 * 7),
                    to: zeroDate
                )!
            )
            let localEvents = EventModel().getCalendarEvents(
                dates: dailyDates,
    ...
    func fullUpdate() {
        ...
        while continueFlag {
            let fourYears = Period.PeriodStartEnd(
                start: kalender.date(
                    byAdding: DateComponents(month: -45),
                    to: whileDate!
                )!,
                end: whileDate!
            )
            let tempEvents = EventModel().getCalendarEvents(
                dates: fourYears,
                eventStore: eventStore
            )
    ...
    func doFirstVisit() {
        let (backLimit, forwardLimit) = DBModel().getVisitMinMax()
        var patientVisits = DBModel().getPatientInfo(
            dates: Period.PeriodStartEnd(start: backLimit!, end: forwardLimit!)
        )
    ...
    func doEventLines(events: [EKEvent]) -> [Event] {
        ...
        for event in events {
            ...
            eventLine.visitCanceled = location.localizedStandardContains(
                "afgezegd"
            )
            eventLine.visitNoShow = location.localizedStandardContains(
                "niet gekomen"
            )
            ...
            if event.hasRecurrenceRules || event.isDetached {
                if event.url == nil {
                    eventLine.id =
                        ...
                        ? UUID().uuidString : event.eventIdentifier
    ...
    func doPatientLines(eventLines: [Event]) -> [PatientInfo] {
        ...
        for event in eventLines {
            ...
            let created = kalender.nextDate(
                after: start,
                matching: startMaandag,
                matchingPolicy: .nextTime,
                direction: .backward
            )!
            let temp = kalender.nextDate(
                after: event.visitDate,
                matching: eindeZondag,
                matchingPolicy: .nextTime,
                direction: .forward
            )!
            let age = max(
                kalender.dateComponents([.weekOfYear], from: created, to: temp)
                    .weekOfYear!,
                0
            )
    ...
    func doChartLines(chartNr: Int = 0) {
        ...
        chartsData.agesVisits = ChartModel().getAgesVisits(
            dates: period.periodDates
        )
        chartsData.agesPatients = ChartModel().getAgesPatients(
            dates: period.periodDates
        )
        chartsData.numbersVisits = ChartModel().getNumbersVisits1(
            dates: period.periodDates
        )

        let togglesMirror = Mirror(reflecting: chartsData.chartToggles).children
            .dropLast()
        var selectedToggles =
            togglesMirror
            .filter { $0.value as! Bool == true && $0.label != "cumulatief" }
            .map {
                chartsData.chartToggles.cumulatief
                    ? $0.label! + "Cum" : $0.label!
            }
    ...
    func doPatientTimeline() {
        patientTimeline = DBModel().getPatientTimeline()
            ...
            .filter {
                let diff = kalender.dateComponents(
                    [.day],
                    from: $0.startDate,
                    to: $0.endDate
                ).day!
    ...
    func doMapView() {
        ...
        if filteredLines.count > 0 {
            mapData = MapModel().doMapView(
                filteredPatients: filteredLines,
